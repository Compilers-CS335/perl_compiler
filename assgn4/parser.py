...
def p_assignment_adv(p):
	...
	if p[3]['type']=="TYPE ERROR":
		...
		if varType==None:
			print "ERROR: line "+str(p.lineno(1))+"  Variable does not exist\n"
...
def p_expression_concat(p):
	...
	p[0] = {'place':symTable.newtmp()}
	if ((p[1]['type']=='STRING') or (p[1]['type']=='RES_STRING')) and ((p[3]['type']=='STRING') or (p[3]['type']=='RES_STRING')) :
		exp_type = 'STRING'
		threeAddrCode.emit(p[0]['place'], p[1]['place'], p[2], p[3]['place'])		
	else:
		print "ERROR: line "+str(p.lineno(2))+" Concatenation operation works only on strings\n"
		exp_type = "TYPE ERROR"
			...
			if type(TAC[3]) is int:
				...
				if global_vars.has_key(temp14):
					...
					if global_vars[temp14]=="NUMBER":
						...
						temp_add[temp_key]=str(offset)+"(%ebp)"
						# num_value_temp=TAC[3]
					elif global_vars[temp14]=="STRING":
				...
				if flag==0 and concat_flag==-1 :
					...
					# print "HAHAHAHA"
					global_vars[TAC[0]]="STRING"
					global_strings[TAC[0].split('$')[1]+"_str__ing__"]=global_strings[TAC[3]]
					string_value_temp=TAC[0].split('$')[1]+"_str__ing__:\n.ascii\t\""+global_strings[TAC[3]]+"\"\n"+TAC[0].split('$')[1] \
					+"_len = . - "+TAC[0].split('$')[1]+"_str__ing__"
					data_strings[str(TAC[0].split('$')[1])+"_str__ing__"]=string_value_temp
					concat_flag=-1
...
############################MULTIPLICATION###############################################
		...
		if TAC[2]=="*":
			if global_vars.has_key(TAC[1]):
				...
				code_string+="movl\t"+get_offset+",%eax\n"
			if global_vars.has_key(TAC[3]):
				code_string+="imull\t"+TAC[3].split('$')[1]+"_num__ber__"+",%eax\n"
			else:
				get_offset=temp_add[str(TAC[3])]
...
############################DIVISION###############################################
		...
		if TAC[2]=="/":
			...
			if global_vars.has_key(TAC[3]):
				code_string+="cdq\n"
				code_string+="idivl\t"+TAC[3].split('$')[1]+"_num__ber__"+"\n"
			else:
				get_offset=temp_add[str(TAC[3])]
				code_string+="cdq\n"
...
################################FUNCTION###################################################
		...
		if TAC[2]==".":
			...
			concat_flag=1
			new_concat=str(global_strings[TAC[1]])+str(global_strings[TAC[3]])
			global_strings[TAC[0]]=new_concat
			# print new_concat
			data_strings[TAC[0]]=TAC[0]+":\n.ascii\t\""+new_concat+"\"\n"+TAC[0]\
							+"_len = . - "+TAC[0]	
